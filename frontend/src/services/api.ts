...
class ApiService {
  ...
  constructor() {
    ...
    // When VITE_API_URL is not set, fall back to the same origin ('/api'),
    // which works for deployments that serve frontend + backend together
    // or use a reverse proxy / rewrite.
    const rawBase = (import.meta as any).env?.VITE_API_URL?.trim();
    const ensureApiSuffix = (u: string) => {
      const cleaned = u.replace(/\/$/, '');
      return cleaned.endsWith('/api') ? cleaned : `${cleaned}/api`;
    };
    const baseURL = rawBase ? ensureApiSuffix(rawBase) : '/api';
